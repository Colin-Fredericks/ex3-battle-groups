$(document).ready(function () {
  ...
  function updateDisplay() {
    ...
    if (stats.attacks) {
      ...
      for (var i = 0, l = stats.attacks.length; i < l; i++) {
        ...
        extraAttacks +=
          '<div class="col1">Attack (' +
          line.type +
          '):</div><div class="pindown">' +
          line.dice;
        if (line.damage) {
  ...
  if (typeof Storage !== 'undefined') {
    if (
      typeof localStorage.BattleGroupSaves !== 'undefined' &&
      ...
      for (var key in savedArmies) {
        ...
        armyMenu.prepend(
          '<option value="' + key + '">' + tempArmy.name + '</option>'
        );
  ...
  $('#save-local').on('click tap', function () {
    ...
    for (var k in savedArmies) {
      if (savedArmies[k].name == stats.name) {
        savedArmies[k] = JSON.stringify(baseStats);
        armies[k] = $.extend(true, {}, baseStats);
